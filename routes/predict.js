...
// Get remedies from Gemini with retry
const getRemediesFromGemini = async (disease, retries = 2) => {
...
Keep it practical and farmer-friendly. If the plant is healthy, provide general care tips instead.`;
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      console.log('Gemini remedies response — finishReason:', candidate?.finishReason);
      const text = candidate?.content?.parts?.find(p => p.text)?.text;
      if (!text) {
        console.error('Gemini remedies — no text in response:', JSON.stringify(response.data).slice(0, 500));
        throw new Error('Empty response from Gemini');
      }
      return text;
    } catch (err) {
      const status = err.response?.status;
      console.error(`Gemini remedies error (attempt ${attempt + 1}) — status: ${status}`, JSON.stringify(err.response?.data?.error || err.message));
...
router.post('/', protect, upload.single('image'), async (req, res) => {
  try {
    ...
    const cached = await Prediction.findOne({
      disease: diseaseName,
      remedies: { $exists: true, $ne: '', $not: /Unable to fetch|consult a local|consult a plant/ }
    });
    if (cached) {
